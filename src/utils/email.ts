...
export async function sendAppointmentEmail(appointment: AppointmentEmailInput) {
  ...
  // finishes (a stale/reused keep-alive socket on the platform side,
  // not a real error from Resend) — a fresh attempt on a new connection
  // almost always succeeds, so retry a couple of times before giving up.
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    try {
      ...
      console.error(
        `sendAppointmentEmail: attempt ${attempt}/${MAX_ATTEMPTS} failed to send notification email:`,
        err
      );
